...
execute unless entity @n[type=item, nbt={ Item: { components: { "minecraft:custom_data": { goldark.items.soul_link: true } } } }, distance=..8] \
        as @p[tag=goldark.paths.werewoof, distance=..12] at @s run function goldark:tools/lycanthropy/cure

execute if entity @n[type=item, nbt={ Item: { components: { "minecraft:custom_data": { goldark.items.soul_link: true } } } }, distance=..8] \
        run function goldark:rituals/lycanthropy_revert/convert_target with entity @n[type=item, nbt={ Item: { components: { "minecraft:custom_data": { goldark.items.soul_link: true } } } }] Item.components.minecraft:custom_data.goldark.soul_link
